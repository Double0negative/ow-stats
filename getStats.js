    ...
    for (var ii = 0; ii < ress.length; ii++) {
      ...
      getStats(useri, overwatch_id, platforms[0], function(user, id, result) {
        if(!result || result.error) {
          getStats(user, id, platforms[1], function(user1, id, result1) {
            if(!result1 || result1.error) {
              getStats(user, id, platforms[2], function(user2, id, result2) {
                if(result2 && !result2.error) {
                  processResults(user2, result2);
                } else {
                  console.log("Not results for ", id, result, result1, result2)
                }
              });
            } else {
              processResults(user1, result1)
            }
          });
        } else {
...
function getStats(user, id, platform,  call) {
  ...
  var url = 'http://' + settings.owapi.host + '/api/v3/u/' + id + '/blob?platform=' + platform;
    request(url, function(err, res, body) {
      ...
      var result;
      console.log("getStats ", url, id, statscount)
      if(body) {
        try{
...
function processResults(user, result) {
  ...
  var query = sql.query("INSERT INTO " + settings.stats.stats_blob + " SET ?", stats_blob, function (err, res, feilds) {
    ...
    for (var key in raw.heroes.playtime.competitive) {
      comp[key] = raw.heroes.playtime.competitive[key];
      winrate[key] =  raw.heroes.stats.competitive[key] ? raw.heroes.stats.competitive[key].general_stats.win_percentage || 0 : 0
    }
    ...
    sql.query("INSERT INTO winrate set ?", winrate, function(err, res, f) {
      if(err)
        throw err
    })
    var stats_ = {
